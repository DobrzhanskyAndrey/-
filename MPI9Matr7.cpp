...
void MatrInit()
{
	...
	{
		pt >> m >> p >> q;
		int sz_a_ = m * p, sz_b_ = p * q, sz_c_ = m * q;
		a_ = new int[sz_a_];
		b_ = new int[sz_b_];
...
void Matr1ScatterData()
{
	...
	na = ceil((double)m / k);
	nb = ceil((double)p / k);
	int sz_a = na * p, sz_b = nb * q, sz_c = na * q;
	a = new int[sz_a];
	b = new int[sz_b];
	c = new int[sz_c];
...
void Matr1Calc(int step)
{
	...
	MPI_Status s;
	MPI_Sendrecv_replace(b, sz_b, MPI_INT, r == 0 ? k - 1 : r - 1, 0, (r + 1) % k, 0, MPI_COMM_WORLD, &s);
}
